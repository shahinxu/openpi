...
def run_episode(
    ...
    image_height=512,
    image_width=512,
    wrong_align_object=None,
    place_success_xy_threshold=0.08,
    first_rotate_enabled=True,
):
    obs = env.reset()
    ...
    wrong_unrotate_steps = 40
    forward_push_steps = 16
    forward_push_distance = 0.084
    forward_push_step_size = forward_push_distance / max(1, forward_push_steps)
    wrong_align_hold_steps = 12
    wrong_raise_total_steps = 14
    align_ready_y_tol = 0.006
    align_ready_z_tol = 0.004
    ...
    wrong_align_obj_joint = None
    if wrong_align_object and wrong_align_object in joints and wrong_align_object != chosen_obj:
        wrong_align_obj_joint = joints[wrong_align_object]

    phase = "wrong_align" if wrong_align_obj_joint is not None else "align"
    rotate_progress = 0
    wrong_rotate_progress = 0
    wrong_unrotate_progress = 0
    forward_progress = 0
    wrong_align_hold_progress = 0
    wrong_raise_progress = 0
    wrong_raise_steps_limit = wrong_raise_total_steps
    ...
    wrong_align_hold_pos = None
    wrong_raise_target_pos = None
    front_ready_count = 0
    rotate_anchor_x = None
    rotate_anchor_pos = None
    wrong_rotate_anchor_pos = None
    wrong_unrotate_anchor_pos = None
    ...
    if wrong_align_obj_joint is not None:
        _wrong_obj_init = get_object_pos_from_joint(env, wrong_align_obj_joint)
        wrong_desired_align_z = float(_wrong_obj_init[2] - 0.06) if _wrong_obj_init is not None else desired_align_z
    else:
        wrong_desired_align_z = desired_align_z
    ...
    while True:
        ...
        if phase in ("wrong_align", "align"):
            if phase == "wrong_align":
                align_obj_now = get_object_pos_from_joint(env, wrong_align_obj_joint)
                align_contact_name = wrong_align_object
                align_target_z = wrong_desired_align_z
            else:
                align_obj_now = obj_now
                align_contact_name = chosen_obj
                align_target_z = desired_align_z

            align_target_y = float(align_obj_now[1] + target_y_offset)
            align_front_target = np.array(
                [align_obj_now[0] + front_sign * front_clearance, align_target_y, align_target_z],
                dtype=np.float64,
            )
            align_robot_can_contact = is_object_in_contact(env, align_contact_name)

            front_ready_enter = hand_is_on_front_side(
                hand_min_x=hand_min_x,
                hand_max_x=hand_max_x,
                obj_x=float(align_obj_now[0]),
                clearance=front_enter_clearance,
                front_sign=front_sign,
            ) and (not align_robot_can_contact)
            ...
            action[0] = 0.0
            action[1] = 0.0
            if align_robot_can_contact or (not front_latched):
                if front_sign > 0:
                    retreat_x = max(base_pos[0] + 0.015, align_obj_now[0] + front_enter_clearance + 0.03)
                else:
                    retreat_x = min(base_pos[0] - 0.015, align_obj_now[0] - front_enter_clearance - 0.03)
                target = np.array([retreat_x, align_front_target[1], align_target_z], dtype=np.float64)
            else:
                target = np.array([base_pos[0], align_front_target[1], align_target_z], dtype=np.float64)

            align_y_err = float(abs(base_pos[1] - align_front_target[1]))
            align_z_err = float(abs(base_pos[2] - align_target_z))
            align_pose_ready = (align_y_err <= align_ready_y_tol) and (align_z_err <= align_ready_z_tol)

            if front_latched and align_pose_ready:
                front_ready_count += 1
            else:
                front_ready_count = 0
            ...
            if front_ready_count >= 8:
                if phase == "wrong_align":
                    if first_rotate_enabled:
                        phase = "wrong_rotate"
                        wrong_rotate_progress = 0
                        wrong_rotate_anchor_pos = base_pos.copy()
                    else:
                        phase = "wrong_align_hold"
                        wrong_align_hold_progress = 0
                        wrong_align_hold_pos = base_pos.copy()
                        wrong_raise_progress = 0
                        front_latched = False
                        front_ready_count = 0
                else:
        ...
        elif phase == "wrong_rotate":
            if wrong_rotate_anchor_pos is None:
                wrong_rotate_anchor_pos = base_pos.copy()
            target = wrong_rotate_anchor_pos.copy()
            action[0] = 0.0
            action[1] = 0.0
            wrong_rotate_progress += 1
            if wrong_rotate_progress >= rotate_total_steps:
                phase = "wrong_unrotate"
                wrong_unrotate_progress = 0
                wrong_unrotate_anchor_pos = base_pos.copy()
                wrong_rotate_anchor_pos = None
                front_latched = False
                front_ready_count = 0

        elif phase == "wrong_unrotate":
            if wrong_unrotate_anchor_pos is None:
                wrong_unrotate_anchor_pos = base_pos.copy()
            target = wrong_unrotate_anchor_pos.copy()
            action[0] = 0.0
            action[1] = 0.0
            wrong_unrotate_progress += 1
            if wrong_unrotate_progress >= wrong_unrotate_steps:
                phase = "wrong_align_hold"
                wrong_align_hold_progress = 0
                wrong_align_hold_pos = base_pos.copy()
                wrong_raise_progress = 0
        ...
        elif phase == "wrong_align_hold":
            if wrong_align_hold_pos is None:
                wrong_align_hold_pos = base_pos.copy()
            target = wrong_align_hold_pos.copy()
            action[0] = 0.0
            action[1] = 0.0
            wrong_align_hold_progress += 1
            if wrong_align_hold_progress >= wrong_align_hold_steps:
                phase = "wrong_raise"
                wrong_align_hold_progress = 0
                wrong_align_hold_pos = None
                wrong_raise_progress = 0
                wrong_raise_target_pos = base_pos.copy()
                # After wrong-object pre-action, return to the episode's initial base height.
                wrong_raise_target_pos[2] = base_initial_z
                z_gap = float(abs(base_initial_z - base_pos[2]))
                dynamic_steps = int(np.ceil(z_gap / max(base_speed, 1e-6))) + 6
                wrong_raise_steps_limit = max(wrong_raise_total_steps, dynamic_steps)

        elif phase == "wrong_raise":
            if wrong_raise_target_pos is None:
                wrong_raise_target_pos = base_pos.copy()
                wrong_raise_target_pos[2] = base_initial_z
                z_gap = float(abs(base_initial_z - base_pos[2]))
                dynamic_steps = int(np.ceil(z_gap / max(base_speed, 1e-6))) + 6
                wrong_raise_steps_limit = max(wrong_raise_total_steps, dynamic_steps)
            target = wrong_raise_target_pos.copy()
            action[0] = 0.0
            action[1] = 0.0
            wrong_raise_progress += 1
            if abs(base_pos[2] - wrong_raise_target_pos[2]) <= 0.003 or wrong_raise_progress >= wrong_raise_steps_limit:
                phase = "align"
                wrong_raise_target_pos = None
                front_latched = False
                front_ready_count = 0
        ...
        if chosen_obj in ("can", "milk"):
            ...
            if phase in ("wrong_align", "align", "rotate", "forward", "grasp", "grasp_lock"):
        ...
        if phase in ("wrong_align", "wrong_rotate", "wrong_unrotate", "wrong_align_hold", "align", "rotate", "forward", "grasp", "grasp_lock"):
            target = target.copy()
            if phase == "wrong_align":
                align_z_cap = wrong_desired_align_z
            elif phase == "wrong_rotate":
                align_z_cap = float(wrong_rotate_anchor_pos[2]) if wrong_rotate_anchor_pos is not None else wrong_desired_align_z
            elif phase == "wrong_unrotate":
                align_z_cap = float(wrong_unrotate_anchor_pos[2]) if wrong_unrotate_anchor_pos is not None else wrong_desired_align_z
            elif phase == "wrong_align_hold":
                align_z_cap = float(wrong_align_hold_pos[2]) if wrong_align_hold_pos is not None else wrong_desired_align_z
            else:
                align_z_cap = desired_align_z
            target[2] = min(target[2], base_pos[2], align_z_cap)

        # Explicit wrist yaw control with front-side hard gate:
        # rotate only after whole hand is in front side and no contact
        if phase in ("wrong_rotate", "wrong_unrotate", "rotate"):
            if phase == "wrong_unrotate":
                alpha = wrong_unrotate_progress / max(1, wrong_unrotate_steps)
                yaw_des = (1.0 - alpha) * yaw_target + alpha * yaw_initial
            elif front_latched:
                if phase == "wrong_rotate":
                    alpha = wrong_rotate_progress / max(1, rotate_total_steps)
                else:
        ...
        elif phase in ("wrong_align", "wrong_rotate", "wrong_unrotate", "wrong_align_hold", "align", "rotate", "grasp", "grasp_lock"):
    ...
    near_end_pos = get_object_pos_from_joint(env, near_target_joint)
    obj_z_end = float(obj_end_pos[2])
    lifted = int(sticky_ever_attached)
    place_xy_distance = float(np.linalg.norm(obj_end_pos[:2] - near_end_pos[:2]))
    place_success = int(place_xy_distance <= float(place_success_xy_threshold))
    ...
    result = {
        ...
        "lift_success": lifted,
        "place_success": int(place_success),
        "place_xy_distance": float(place_xy_distance),
...
def main():
    ...
    parser.add_argument("--place-success-xy-threshold", type=float, default=0.08, help="Success if final XY distance(target, near-object) <= this threshold")
    parser.add_argument("--wrong-align-object", type=str, default=None, help="Object to initially (incorrectly) align to before correcting to the actual target object")
    parser.add_argument("--first-rotate", dest="first_rotate", action="store_true", help="Enable first-stage rotate for wrong-align pre-action")
    parser.add_argument("--no-first-rotate", dest="first_rotate", action="store_false", help="Disable first-stage rotate for wrong-align pre-action")
    ...
    parser.add_argument("--episode-images", dest="episode_images", action="store_true", help="Store agentview images in episode output")
    ...
    parser.set_defaults(first_rotate=True)
    ...
    print("Place success XY threshold:", args.place_success_xy_threshold)
    print("Wrong align object:", args.wrong_align_object)
    print("First-stage rotate:", bool(args.first_rotate))
    ...
    with h5py.File(output, "w") as f:
        ...
        for ep in range(args.episodes):
            ...
            ep_result = run_episode(
                ...
                wrong_align_object=args.wrong_align_object,
                place_success_xy_threshold=args.place_success_xy_threshold,
                first_rotate_enabled=bool(args.first_rotate),
            )
            ...
            grp.attrs["place_success"] = int(ep_result["place_success"])
            grp.attrs["place_xy_distance"] = float(ep_result["place_xy_distance"])
