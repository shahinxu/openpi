...
class Device(metaclass=abc.ABCMeta):
    ...
    def input2action(self, mirror_actions=False, goal_update_mode="target") -> Optional[Dict]:
        ...
        gripper_dof = robot.gripper[active_arm].dof if gripper is not None else 0
        ...
        for arm in robot.arms:
            ...
            arm_gripper_dof = robot.gripper[arm].dof if robot.gripper[arm] is not None else 0
            ac_dict[f"{arm}_gripper"] = np.zeros(arm_gripper_dof)
        ...
        if gripper is not None:
            ...
            # No gripper for this robot (e.g., Hannes)
            ac_dict[f"{active_arm}_gripper"] = np.array([])
    ...
    def get_arm_action(self, robot, arm, norm_delta, goal_update_mode="target"):
        ...
        elif robot.composite_controller_config["type"] in ["BASIC"]:
            # For BASIC controllers (like Hannes), just pass through the normalized delta
            # as the controller expects joint-level commands
            delta_action = norm_delta.copy()
            # Scale delta action appropriately for joint position control
            delta_action *= 0.2  # Increased from 0.05 for more responsive control
            
            # For BASIC controllers, abs action is the same as delta since
            # we don't have Cartesian space targets
