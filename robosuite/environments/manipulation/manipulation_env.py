...
class ManipulationEnv(RobotEnv):
    ...
    def _check_grasp(self, gripper, object_geoms):
        ...
        if isinstance(gripper, GripperModel):
            ...
            assert all([gripper[arm] is None or isinstance(gripper[arm], GripperModel) for arm in gripper]), "Invalid gripper dict format!"
            # Filter out None grippers and check if any valid gripper is grasping
            valid_grippers = {arm: gripper[arm] for arm in gripper if gripper[arm] is not None}
            if not valid_grippers:
                return False  # No valid grippers means no grasping
            return any([self._check_grasp(valid_grippers[arm], object_geoms) for arm in valid_grippers])
        else:
    ...
    def _gripper_to_target(self, gripper, target, target_type="body", return_distance=False):
        ...
        if isinstance(gripper, dict):
            assert all([gripper[arm] is None or isinstance(gripper[arm], GripperModel) for arm in gripper]), "Invalid gripper dict format!"
            # Filter out None grippers
            valid_grippers = {arm: gripper[arm] for arm in gripper if gripper[arm] is not None}
            if not valid_grippers:
                # No valid grippers, return a large distance or zero vector
                return float('inf') if return_distance else np.array([float('inf')] * 3)
            # get the min distance to the target if there are multiple arms
            if return_distance:
                return min(
                    [self._gripper_to_target(valid_grippers[arm], target, target_type, return_distance) for arm in valid_grippers]
                )
            else:
                return min(
                    [self._gripper_to_target(valid_grippers[arm], target, target_type, return_distance) for arm in valid_grippers],
    ...
    def _visualize_gripper_to_target(self, gripper, target, target_type="body"):
        ...
        if isinstance(gripper, dict):
            assert all([gripper[arm] is None or isinstance(gripper[arm], GripperModel) for arm in gripper]), "Invalid gripper dict format!"
            # Only visualize valid grippers
            valid_grippers = {arm: gripper[arm] for arm in gripper if gripper[arm] is not None}
            for arm in valid_grippers:
                self._visualize_gripper_to_target(valid_grippers[arm], target, target_type)
            return
        # Get gripper and target positions
        gripper_pos = self.sim.data.get_site_xpos(gripper.important_sites["grip_site"])
        # If target is MujocoModel, grab the correct body as the target and find the target position
