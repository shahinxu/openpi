#!/bin/bash
# Collect grip_6 episodes for can / milk.
# Each episode is saved to its own file. Failed episodes are retried.
# Usage: bash run_collect_grip6_batch.sh [episodes_per_target]
...
for TARGET in "${TARGETS[@]}"; do
    existing_max=$(find "$OUTDIR" -maxdepth 1 -type f -name "grip6_${TARGET}_*.hdf5" \
    ...
    for ep in $(seq 1 "$EPISODES_PER_TARGET"); do
        ...
        OUTFILE="${OUTDIR}/grip6_${TARGET}_${IDX}.hdf5"
        ...
        while [ "$success" -eq 0 ]; do
            ...
            "$PYTHON_BIN" collect_hannes_autocruise_grip_6.py \
                --episodes 1 \
