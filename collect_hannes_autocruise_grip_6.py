...
def compose_base_yaw(base_quat, yaw_rad):
    # Apply yaw in the base local frame so turning keeps body orientation consistent.
    quat = quat_wxyz_multiply(base_quat, quat_wxyz_from_yaw(yaw_rad))
...
def run_episode(
    ...
    align_speed=0.012,
    align_blend_steps=10,
    align_ready_count_required=5,
    sticky_after_close=False,
    look_back_total_deg=180.0,
    look_back_turn_steps=36,
    look_back_hold_steps=10,
    look_back_return_hold_steps=12,
    arm_rot_noise_refresh_steps=18,
    arm_rot_noise_alpha=0.22,
    arm_rot_noise_rx_deg=12.0,
    arm_rot_noise_ry_deg=16.0,
    arm_rot_noise_rz_deg=36.0,
    image_height=512,
    image_width=512,
):
    obs = env.reset()

    base_quat_nominal = np.array([0.707, 0.0, 0.0, -0.707], dtype=np.float64)
    base_quat = base_quat_nominal.copy()
    ...
    arm_rot_noise_current = np.zeros(3, dtype=np.float64)
    arm_rot_noise_target = np.zeros(3, dtype=np.float64)
    arm_rot_noise_amp = np.deg2rad(
        np.array([arm_rot_noise_rx_deg, arm_rot_noise_ry_deg, arm_rot_noise_rz_deg], dtype=np.float64)
    )
    front_ready_count = 0
    ...
    align_contact_retreat_steps = 3
    align_y_tol = 0.014
    align_z_tol = 0.010
    approach_far_progress = 0
    look_back_progress = 0
    look_back_hold_progress = 0
    look_back_return_hold_progress = 0
    align_blend_progress = 0
    align_blend_start_pos = None
    align_blend_target_pos = None
    look_back_yaw_rad = float(np.deg2rad(max(0.0, look_back_total_deg)))
    staging_target = None
    ...
    for t in range(step_limit):
        ...
        if t % max(1, int(arm_rot_noise_refresh_steps)) == 0:
            arm_rot_noise_target = rng.uniform(-arm_rot_noise_amp, arm_rot_noise_amp)
        arm_rot_noise_current += float(arm_rot_noise_alpha) * (arm_rot_noise_target - arm_rot_noise_current)
        ...
        if phase == "approach_far":
            ...
            if approach_far_progress < max(0, int(approach_turn_noise_steps)):
                ...
                arm_rot_phase_override = clip_joint_targets(
                    arm_rot_initial + arm_rot_noise_current + np.array([0.0, ry_walk, rz_noise], dtype=np.float64),
                    arm_rot_low,
                    arm_rot_high,
                )
            else:
                arm_rot_phase_override = clip_joint_targets(
                    arm_rot_initial + arm_rot_noise_current + np.array([0.0, ry_walk, 0.0], dtype=np.float64),
                    arm_rot_low,
                    arm_rot_high,
                )
            ...
            if dist_to_stage <= 0.05:
                if look_back_yaw_rad > 1e-6 and look_back_turn_steps > 0:
                    phase = "look_back_turn"
                    look_back_progress = 0
                else:
                    phase = "align"

        elif phase == "look_back_turn":
            target = staging_target.copy() if staging_target is not None else base_pos.copy()
            action[0] = 0.0
            action[1] = 0.0
            look_alpha = min(1.0, look_back_progress / max(1, look_back_turn_steps - 1))
            base_quat = compose_base_yaw(base_quat_nominal, look_back_yaw_rad * look_alpha)
            arm_rot_phase_override = clip_joint_targets(
                arm_rot_initial + 0.85 * arm_rot_noise_current,
                arm_rot_low,
                arm_rot_high,
            )
            look_back_progress += 1
            if look_back_progress >= look_back_turn_steps:
                phase = "look_back_hold"
                look_back_hold_progress = 0

        elif phase == "look_back_hold":
            target = staging_target.copy() if staging_target is not None else base_pos.copy()
            action[0] = 0.0
            action[1] = 0.0
            base_quat = compose_base_yaw(base_quat_nominal, look_back_yaw_rad)
            arm_rot_phase_override = clip_joint_targets(
                arm_rot_initial + 0.85 * arm_rot_noise_current,
                arm_rot_low,
                arm_rot_high,
            )
            look_back_hold_progress += 1
            if look_back_hold_progress >= max(0, int(look_back_hold_steps)):
                phase = "look_back_return"
                look_back_progress = 0

        elif phase == "look_back_return":
            target = staging_target.copy() if staging_target is not None else base_pos.copy()
            action[0] = 0.0
            action[1] = 0.0
            look_alpha = min(1.0, look_back_progress / max(1, look_back_turn_steps - 1))
            base_quat = compose_base_yaw(base_quat_nominal, look_back_yaw_rad * (1.0 - look_alpha))
            arm_rot_phase_override = clip_joint_targets(
                arm_rot_initial + 0.35 * arm_rot_noise_current,
                arm_rot_low,
                arm_rot_high,
            )
            look_back_progress += 1
            if look_back_progress >= look_back_turn_steps:
                phase = "look_back_return_hold"
                look_back_return_hold_progress = 0

        elif phase == "look_back_return_hold":
            target = staging_target.copy() if staging_target is not None else base_pos.copy()
            action[0] = 0.0
            action[1] = 0.0
            base_quat = base_quat_nominal.copy()
            arm_rot_phase_override = clip_joint_targets(
                arm_rot_initial + 0.12 * arm_rot_noise_current,
                arm_rot_low,
                arm_rot_high,
            )
            look_back_return_hold_progress += 1
            if look_back_return_hold_progress >= max(0, int(look_back_return_hold_steps)):
                # Reset to canonical pre-grasp posture before entering align.
                arm_rot_hold_des = arm_rot_initial.copy()
                arm_rot_noise_current[:] = 0.0
                arm_rot_noise_target[:] = 0.0
                front_latched = False
                front_ready_count = 0
                align_contact_count = 0
                align_blend_progress = 0
                align_blend_start_pos = base_pos.copy()
                align_blend_target_pos = front_target.copy()
                phase = "align_blend"

        elif phase == "align_blend":
            # Smoothly connect look-back return to the align phase in position + arm pose.
            action[0] = 0.0
            action[1] = 0.0
            blend_alpha = min(1.0, align_blend_progress / max(1, int(align_blend_steps) - 1))
            if align_blend_target_pos is None:
                align_blend_target_pos = front_target.copy()
            if align_blend_start_pos is None:
                align_blend_start_pos = base_pos.copy()
            target = (1.0 - blend_alpha) * align_blend_start_pos + blend_alpha * align_blend_target_pos
            arm_rot_phase_override = interpolate_joint_targets(arm_rot_hold_des, arm_rot_initial, blend_alpha)
            align_blend_progress += 1
            if align_blend_progress >= max(1, int(align_blend_steps)):
                phase = "align"
            ...
            action[0] = 0.0
            action[1] = 0.0
            # Keep a stable arm orientation in align, otherwise front-side checks drift.
            arm_rot_phase_override = arm_rot_initial.copy()
            ...
            allow_rotate = front_ready_count >= max(1, int(align_ready_count_required))
        ...
        elif phase in ("align_blend", "align"):
        ...
        elif phase in ("approach_far", "look_back_turn", "look_back_hold", "look_back_return", "look_back_return_hold", "align_blend", "align", "rotate", "grasp", "grasp_lock"):
...
def main():
    ...
    parser.add_argument("--align-speed", type=float, default=0.012)
    parser.add_argument("--align-blend-steps", type=int, default=10)
    parser.add_argument("--align-ready-count-required", type=int, default=5)
    parser.add_argument("--sticky-after-close", action="store_true", default=True)
    parser.add_argument("--look-back-total-deg", type=float, default=180.0)
    parser.add_argument("--look-back-turn-steps", type=int, default=36)
    parser.add_argument("--look-back-hold-steps", type=int, default=10)
    parser.add_argument("--look-back-return-hold-steps", type=int, default=12)
    parser.add_argument("--arm-rot-noise-refresh-steps", type=int, default=18)
    parser.add_argument("--arm-rot-noise-alpha", type=float, default=0.22)
    parser.add_argument("--arm-rot-noise-rx-deg", type=float, default=12.0)
    parser.add_argument("--arm-rot-noise-ry-deg", type=float, default=16.0)
    parser.add_argument("--arm-rot-noise-rz-deg", type=float, default=36.0)
    ...
    with h5py.File(output, "w") as f:
        ...
        for ep in range(args.episodes):
            ...
            ep_result = run_episode(
                ...
                align_speed=args.align_speed if hasattr(args, "align_speed") else 0.012,
                align_blend_steps=args.align_blend_steps,
                align_ready_count_required=args.align_ready_count_required,
                sticky_after_close=args.sticky_after_close if hasattr(args, "sticky_after_close") else False,
                look_back_total_deg=args.look_back_total_deg,
                look_back_turn_steps=args.look_back_turn_steps,
                look_back_hold_steps=args.look_back_hold_steps,
                look_back_return_hold_steps=args.look_back_return_hold_steps,
                arm_rot_noise_refresh_steps=args.arm_rot_noise_refresh_steps,
                arm_rot_noise_alpha=args.arm_rot_noise_alpha,
                arm_rot_noise_rx_deg=args.arm_rot_noise_rx_deg,
                arm_rot_noise_ry_deg=args.arm_rot_noise_ry_deg,
                arm_rot_noise_rz_deg=args.arm_rot_noise_rz_deg,
            )
